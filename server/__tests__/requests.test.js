...
describe("GET /requests", () => {
  beforeAll(async () => {
    ...
    lastInsertedUser = await usersCollection.find({}).sort({ _id: -1 }).limit(1).toArray()
    ...
    newestRequest = await requestsCollection.find({}).sort({ _id: -1 }).limit(1).toArray()
  })
    ...
    request(app)
      ...
      .send({
        ...
        headers: [
          {
            key: "access_token",
            value: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MiwiZW1haWwiOiJlbWFpbDJAZ21haWwuY29tIiwicm9sZSI6ImFkbWluIiwiaWF0IjoxNjM2Nzk1ODYyfQ.fHc7RcbOY7Dqqde12_j9IupjuVYtmxOIZ7GBY1UyNWU",
          },
        ],
      })
      .then((response) => {
    ...
    request(app)
      ...
      .send({
        ...
        headers: [
          {
            key: "access_token",
            value: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MiwiZW1haWwiOiJlbWFpbDJAZ21haWwuY29tIiwicm9sZSI6ImFkbWluIiwiaWF0IjoxNjM2Nzk1ODYyfQ.fHc7RcbOY7Dqqde12_j9IupjuVYtmxOIZ7GBY1UyNW",
          },
        ],
      })
      .then((response) => {
  ...
  test("[failed] failed read json file because the file type is not json", (done) => {
    request(app)
      ...
      .then((response) => {
        ...
        expect(body).toEqual(expect.any(Object))
        expect(body).toHaveProperty("message", "File type needs to be application/json")
